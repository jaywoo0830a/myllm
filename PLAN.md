...
## 목표
**DeepSeek-R1-Distill-Qwen-32B (단일)** 을 **별도 베어메탈 서버**(9700X / 64GB RAM, Ubuntu, GPU 없음)에서
...
## 현재 모델 (단일, config/models/deepseek-r1-32b.env)
| slug | GGUF | 크기 | 포트 |
|------|------|------|------|
| `deepseek-r1-32b` | `unsloth/DeepSeek-R1-Distill-Qwen-32B-GGUF` `...-Q4_K_M.gguf` | ~19.9GB | 8081 |

## 실제 모델 사실 (2026-09 검증)
- `deepseek-ai/DeepSeek-R1-Distill-Qwen-32B`, GGUF `unsloth` repo, arch **`qwen2`**(안정).
- **dense ~32.8B**(64 layers, GQA) — NOT MoE. 컨텍스트 네이티브 131072.
- R1 템플릿 + **항상 `<think>` 먼저, 억제 불가**(NO_THINK 무효).
- **속도**: dense 32B Q4 = 대역폭 병목 → 9700X CPU에서 약 **2.5-4 t/s**(실측 2 t/s 근처).
  긴 CoT 포함 시 답 하나에 수 분. RAM 64GB에 fit(총 ~27-31GB).

## 예상 속도/운영 (서버 CPU)
- 단일 모델 → 8코어 전부/대역폭 전용 (THREADS=8 물리코어, SMT 16은 손해).
- KV_CACHE q8_0 → 소폭 개선 시도. 긴 문서는 `overnight_gen.sh`(백그라운드) 추천.
- reasoning(`<think>`)이 길어 체감 대기 큼 — 필요 시 클라이언트에서 `</think>` 뒤만 표시.
...
[베어메탈 서버 9700X · 64GB · Ubuntu · GPU 없음]
   llama-server x1  (deepseek-r1-32b, full 8코어)
      └ DeepSeek-R1-Distill-Qwen-32B-Q4_K_M.gguf (~19.9GB) @8081
```
...
- **server/config/models/deepseek-r1-32b.env(.example)** 모델 프로파일 (단일)
- **server/systemd/myllm-llama@.service** systemd 템플릿
- **client/continue/config.yaml.example** 단일 모델 apiBase 템플릿

## 구현 단계 (완료/남음)
- [x] 단일 모델 프로파일(deepseek-r1-32b) + 공통 env 분리 (deepseek-1.5b 제거)
